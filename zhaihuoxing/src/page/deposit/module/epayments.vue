    ...
    .user-operation
      el-form(:model="epayments", :rules="epaymentsRules", ref="form", label-width="140px", label-position="left")
        ...
        el-form-item(prop="depositAmount")
          ...
          el-input.input-long-type2(:placeholder="renderData.pleaseEnter", v-model="epayments.depositAmount", :disabled="epayments.tradeAccount===''")
...
<script>
  ...
  export default {
    ...
    name: 'epayments',
    data () {
      let [_this, renderData] = [this, window.renderData.components.deposit.deposit_epayments]
      return {
        currencySymbol: '-',
        description: {
          attention: renderData.depositEpaymentsAttention,
          text: renderData.depositEpaymentsText
        },
      ...
      submitForm () {
        ...
        vm.$refs['bFormItemValue'].validate((valid, data) => {
          if (valid && JSON.stringify(data.others) !== '{}') {
            Object.keys(data.others).forEach(function (key) {
              vm.epayments.others[key] = data.others[key]
            })
          }
        ...
        if (res) {
          let params = {
            ...
            amount: vm.epayments.depositAmount,
            currency: vm.epayments.currency,
            others: JSON.parse(JSON.stringify(vm.epayments.others))
          }
          service.depositEpayments(params).then(resp => {
            console.log(resp)
          })
        }
      },
      getCurrency (currency, item) {
        this.epayments.aid = item.aid
