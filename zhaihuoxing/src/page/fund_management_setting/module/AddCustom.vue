<template lang="pug">
  b-dialog.add_custom(:show='true', :title="renderData.customFieldEdit", width="38%", :show-close="true", :before-close="beforeClose")
    .status_one(v-show="isShow")
      el-form(:model="resourceInfo", ref="resourceInfo", label-width="140px", label-position="left")
    ...
    methods: {
      ...
      async saveResource () {
        ...
        if (this.childValidRes) {
          ...
          var somedata = vm.data
          var idx = this.idx
          this.$emit('sendData', somedata, idx)
          this.visible.dialog = null
        }
      },
