  ...
  #withdrawal-setting
    el-form.statusForm(ref="statusForm", :rules="statusForm.statusRules", :model="statusForm", label-width="140px", label-positon="left")
      el-form-item.theme-color-C.status
        template(slot="label")
          span.theme-color-C.inline-label(v-text="renderData.withdrawalStatusSetting", v-ellipsis-title="")
      ...
      el-collapse-item.theme-color-C(v-for="(item, parIndex) in dataList", :key="parIndex", :title="renderData[item.name]", :name="item.name")
        el-form(:ref="item.name", :model="item", label-width="140px", label-position="left")
          ...
          el-form-item.default_comment(prop="data.comment.defaultComment.label", :rules="depositRules.defaultComment")
            template(slot="label")
              span.theme-color-C.inline-label(v-text="renderData.defaultComment")
            b-input.input(:placeholder="renderData.pleaseEnter", :model.sync="item.data.comment.defaultComment.label")
          el-form-item(v-if="item.data.comment.otherComment")
            template(slot="label")
            ...
            .other-item(v-for="(item1, idx) in item.data.comment.otherComment", :key="item1.key")
              el-form(ref="otherform", :model="item1")
                ...
                //el-form-item(:prop="'otherComment.' + idx + '.label'", :rules="depositRules.otherComment")
                  b-input.other-input(:placeholder="renderData.pleaseEnter", :model.sync="item1.label")
                  b-icon.theme-color-darkenC15.theme-color-lightenA10-hover.theme-color-darkenA10-active.delete(iconName="delete1", @click.native="deleteother(item,idx)")
            .other-item
        ...
        .footer
          b-button(type="primary", @click="saveform(item)") {{renderData.save}}
...
<script>
  ...
  export default {
    name: 'withdrawal-setting',
    data () {
      var fundSetting = window.renderData.components.fund_management_setting
      var renderData = Object.assign({}, fundSetting, fundSetting.fund_management_setting_withdrawal)
      return {
    ...
    methods: {
          ...
          if (!error) {
            ...
            service.addStatus(params).then(({re, data}) => {
              ...
              return service.getFundStatus(params).then(data => {
                this.statusForm.status = data.withdrawalStatus
              })
            })
          }
        })
      ...
      getFundWithdrawalStatus () {
        this.statusForm.status = this.StatusData.withdrawalStatus
      },
      closeInput () {
      ...
      getFundSetting () {
        this.dataList = this.Data.withdrawal
        this.activeName.push(this.Data.withdrawal[0].name)
      },
    ...
    async mounted () {
      await this.getFundWithdrawalStatus()
