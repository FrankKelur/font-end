...
<script>
  ...
  export default {
    ...
    methods: {
      uploadCDN () {
        // 检查formItemList 如果里面有需要上传的文件，上传文件，且生成参数时，替换tmp-preview为 preview
        var filenames = []
        this.formItemList.forEach(item => {
          if (item.type === 'upload') {
            filenames.push(this.model[item.key].url.replace('tmp-preview', ''))
            this.model[item.key].url = this.model[item.key].url.replace('tmp-preview', 'preview')
          }
        })
        if (filenames.length) {
          var params = {
            originPath: filenames
          }
...
//          console.log('custom-form valid ?', valid)
//        })
